...
. Tworzenie projektów (jobs, konfiguracja repozytorium, skrypty budowania i testowania)
. Pipeliny (deklaratywne i skryptowe)

=== Dzień 2
. Integracja z narzędziami zewnętrznymi m.in. git, maven gradle
. Powiadomienia
. Jak zautomatyzować proces wdrażania
. Wdrażanie na różne środowiska (testowe, produkcyjne)
